...
import 'package:cloud_firestore/cloud_firestore.dart'; // Required for saving keys
...
Future<Map<String, dynamic>> uploadUserFiles(
    ...
    await Future.wait(files.map((file) async {
        ...
        try {
            ...
            String fileKey = EncryptionService.generateFileKey();
            Uint8List encryptedBytes = EncryptionService.encryptData(fileBytes, fileKey);

            // 3. UPLOAD ENCRYPTED BYTES TO STORAGE
            final ref = storage.ref().child('user_data/$userEmail/${file.name}');
            // Use putData instead of putFile because we have bytes in memory
            await ref.putData(encryptedBytes);

            // 4. SAVE KEY TO FIRESTORE (The "Vault")
            // This is crucial: without this, the file is lost forever!
            await firestore
              .collection('file_keys')
              .doc(userEmail)
              .collection('keys') // Optional: use a sub-collection for all keys
              .doc(file.name)     // The document name IS the file name
              .set({
                  'fileName': file.name,
    ...
    return uploadedCount > 0 
        ? {'success': true, 'message': 'Uploaded $uploadedCount files.'}
